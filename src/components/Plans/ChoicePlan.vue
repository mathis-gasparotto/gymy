<template>
  <div class="flex column">
    ...
    <div v-if="plans && plans.length > 0">
      <draggable :list="plans" class="list-group" ghost-class="ghost" itemKey="id" handle=".draggable-btn" @end="onDragEnd" @start="drag=true">
        <template #item="{ element }">
          <q-card @click="$emit('selectPlan', element)" class="cursor-pointer q-mb-md flex-center column q-px-md" >
    ...
    <span v-else class="text-center">Aucune planification de disponible</span>
    <q-dialog v-model="editForm">
      <q-card class="q-px-xs q-py-xs">
        ...
        <q-card-section>
          <PlanForm :initData="planToEdit" buttonLabel="Confirmer" :loading="editLoading" @submit="onEditSubmit" />
        </q-card-section>
    ...
    <q-dialog v-model="addForm">
      <q-card class="q-px-xs q-py-xs">
        ...
        <q-card-section>
          <PlanForm buttonLabel="Ajouter" buttonIcon="add" :loading="addLoading" @submit="onAddSubmit" />
        </q-card-section>
        <q-card-actions align="center">
          <q-btn label="Annuler" color="negative" v-close-popup />
        </q-card-actions>
