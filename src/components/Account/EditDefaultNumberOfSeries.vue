<template>
  <div>
    <q-btn
      ...
      color="primary"
      class="w-content"
      >Modifier mon nombre de séries par défaut</q-btn
    >
        ...
        >
          ...
          >
            <q-input
              ...
              min="1"
              :rules="[(val) => (val !== null && val !== '') || 'Veuillez remplir ce champ', (val) => val >= 1 || 'Veuillez renseigner une valeur positive supérieur à 0']"
...
export default {
  ...
  methods: {
    submit() {
      ...
      this.loading = true
      updateUser({ defaultNumberOfSeries: Number(this.newDefaultNumberOfSeries) })
        .then(() => {
